...
namespace GMTOOL
{
Convertor::Convertor(QWidget* parent) : QWidget(parent), ui(new Ui::Convertor), m_customRulesModel(new CustomRuleModel)
{
    ...
    m_catModel.reset(new CategoryModel());
    ...
    if(m_model->rowCount(QModelIndex()) == 0)
    {
        ...
        m_convertorTable.insert(QPair<const Unit*, const Unit*>(kelvin, fahrenheit),
                                new ConvertorOperator(9.0 / 5.0, -459.67));
        ...
        m_convertorTable.insert(QPair<const Unit*, const Unit*>(cubicFeet, cubicMeter),
                                new ConvertorOperator(0.0283168));
    }

    m_catModel->setSourceModel(m_model.get());
    m_toModel->setSourceModel(m_model.get());
    // editCatModel->setSourceModel(m_model);

    m_customRulesModel->setSourceModel(m_model.get());
    categoryHasChangedOnSecondPanel(0);
    m_customRulesModel->setConvertionRules(&m_convertorTable);

    ui->m_tableView->setModel(m_customRulesModel.get());
...
void Convertor::writeSettings()
{
    ...
    {
        auto unit= m_model->getUnitByIndex(i);
        setting.setArrayIndex(i);
