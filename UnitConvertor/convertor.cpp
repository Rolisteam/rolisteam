...
namespace GMTOOL
{
    Convertor::Convertor(QWidget* parent)
        : QWidget(parent), ui(new Ui::Convertor), m_customRulesModel(new CustomRuleModel)
    {
        ...
        connect(
            ui->m_categoryCombo2, SIGNAL(currentIndexChanged(int)), this, SLOT(categoryHasChangedOnSecondPanel(int)));
        ...
        if(m_model->rowCount(QModelIndex()) == 0)
        {
            ...
            Unit* kelvin
                = m_model->insertData(new Unit(QStringLiteral("kelvin"), QStringLiteral("K"), Unit::TEMPERATURE));
            Unit* fahrenheit
                = m_model->insertData(new Unit(QStringLiteral("fahrenheit"), QStringLiteral("°F"), Unit::TEMPERATURE));

            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(celsius, kelvin), new ConvertorOperator(1, -273.15));
            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(celsius, fahrenheit), new ConvertorOperator(1.8, 32));
            m_convertorTable.insert(QPair<const Unit*, const Unit*>(kelvin, celsius), new ConvertorOperator(1, 273.15));
            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(fahrenheit, celsius), new ConvertorOperator(1.8, -32, true));

            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(fahrenheit, kelvin), new ConvertorOperator(9.0 / 5.0, 459.67, true));
            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(kelvin, fahrenheit), new ConvertorOperator(9.0 / 5.0, -459.67));
            ...
            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(gram, pound), new ConvertorOperator(1.0 / 453.59237));
            ...
            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(kilo, pound), new ConvertorOperator(1.0 / 0.45359237));
            ...
            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(cubicMeter, cubicFeet), new ConvertorOperator(35.3147));
            m_convertorTable.insert(
                QPair<const Unit*, const Unit*>(cubicFeet, cubicMeter), new ConvertorOperator(0.0283168));
        }
        ...
        m_customRulesModel->setSourceModel(m_model);
