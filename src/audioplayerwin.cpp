...
{
    ((AudioPlayer *)userdata)->arriveeEnFinDeTitre();
    return true;
}
...
{
    ((AudioPlayer *)userdata)->passageSurUnTag(QString((char *)buff));
    return true;
}
...
AudioPlayer*  AudioPlayer::getInstance(QWidget *parent)
 {
	...
	/********************************************************************/
    void AudioPlayer::autoriserOuIntedireCommandes()
	{
	...
	/********************************************************************/
    void AudioPlayer::appuiPause(bool etatBouton)
	{
		...
		else
		{
			...
			{
                qWarning("Le bouton pause etait enfonce alors que le lecteur n'etait pas en pause (appuiPause - AudioPlayer.cpp)");
				return;
			}
		}
	}
	...
	/********************************************************************/
    void AudioPlayer::appuiStop(bool etatBouton)
	{
	...
	/********************************************************************/
    void AudioPlayer::appuiLecture(bool etatBouton)
	{
		// Le bouton vient d'etre enfonce
		if (etatBouton)
		{
		...
		else
		{
			...
			{
                qWarning("Le bouton lecture etait enfonce alors que le lecteur n'etait pas en en train de lire (appuiLecture - AudioPlayer.cpp)");
				return;
			}
		}
	}
	...
	/* en boucle                                                        */
	/********************************************************************/
    void AudioPlayer::appuiBoucle(bool etatBouton)
	{
		// On lit en boucle
		enBoucle = etatBouton;
	...
	/* titre suivant)                                                   */
	/********************************************************************/
    void AudioPlayer::appuiUnique(bool etatBouton)
	{
	...
	/********************************************************************/
    void AudioPlayer::supprimerTitre()
	{
		...
		if (ligne == titreCourant)
		{
			...
			{
                qWarning("Probleme a la fermeture d'un flux audio (supprimerTitre - AudioPlayer.cpp)");
				return;
			}
	...
	/* niveau sonore                                                    */
	/********************************************************************/
    void AudioPlayer::changementVolume(int valeur)
	{
	...
	/********************************************************************/
    void AudioPlayer::changementTempsLecture()
	{
	...
	/********************************************************************/
    void AudioPlayer::changementTempsAffichage(int valeur)
	{
	...
	/********************************************************************/
    void AudioPlayer::changementTitre(QListWidgetItem *morceau)
	{
	...
	/********************************************************************/
    void AudioPlayer::nouveauTitre(QString titre, QString fichier)
	{
		...
		if (fluxAudio)
		{
			...
			if (!FSOUND_Stream_Close(fluxAudio))
			{
                qWarning("Probleme a la fermeture d'un flux audio (nouveauTitre - AudioPlayer.cpp)");
				return;
			}
		}
		...
		if (!fluxAudio)
		{
            qWarning("Impossible d'ouvrir le fichier audio (nouveauTitre - AudioPlayer.cpp)");
			return;
		}
		...
		{
            qWarning("Impossible d'initialiser la fonction de callback de fin de lecture (nouveauTitre - AudioPlayer.cpp)");
			return;
		}
		...
		{
            qWarning("Impossible d'initialiser la fonction de callback de passage sur un TAG (nouveauTitre - AudioPlayer.cpp)");
			return;
		}
	...
	/********************************************************************/
    void AudioPlayer::arriveeEnFinDeTitre()
	{
	...
	/* callback                                                         */
	/********************************************************************/
    void AudioPlayer::finDeTitreSlot()
	{
    ...
    void AudioPlayer::ajouterTags()
	{
		...
		while(secondes*1000 < longueurMorceau)
		{
			...
			if (!FSOUND_Stream_AddSyncPoint(fluxAudio, secondes * FREQUENCE_AUDIO, stringTag.toLatin1().data()))
                qWarning ("Impossible de creer un point de synchronisation (ajouterTags - AudioPlayer.cpp)");
	...
	/********************************************************************/
    void AudioPlayer::passageSurUnTag(QString tag)
	{
	...
	/* les clients                                                      */
	/********************************************************************/
    void AudioPlayer::emettreCommande(actionMusique action, QString nomFichier, quint32 position, int numeroLiaison)
	{
		int p;
	...
	/********************************************************************/
    void AudioPlayer::emettreEtat(QString idJoueur)
	{
		...
		switch(etatActuel)
		{
			...
			default :
                qWarning("Etat du lecteur audio inconnu lors de l'emission de l'etat (emettreEtat - AudioPlayer.cpp)");
				break;
		}
	}
	...
	/********************************************************************/
    void AudioPlayer::joueurNouveauFichier(QString nomFichier)
	{
		// Si un fichier etait en cours de lecture, on le ferme
		if (fluxAudio)
		{
			...
			if (!FSOUND_Stream_Close(fluxAudio))
			{
                qWarning("Probleme a la fermeture d'un flux audio (joueurNouveauFichier - AudioPlayer.cpp)");
				return;
			}
		}
	...
	/********************************************************************/
    void AudioPlayer::joueurLectureMorceau()
	{
		// Si aucun fichier audio n'est actuellement ouvert, on quitte la fonction
		if (!fluxAudio)
		{
            qWarning("Demande de lecture alors qu'aucun fichier n'est ouvert (joueurLectureMorceau - AudioPlayer.cpp)");
			return;
		}
		...
		// Dans tous les autres cas il s'agit d'une erreur
            ...
            qWarning("Demande de lecture d'un titre alors que le lecteur n'est ni en pause, ni a l'arret (joueurLectureMorceau - AudioPlayer.cpp)");
	}

	/********************************************************************/
	/* Joueur seulement - met la lecture en pause                       */
	/********************************************************************/
    void AudioPlayer::joueurPauseMorceau()
	{
		// Si aucun fichier audio n'est actuellement ouvert, on quitte la fonction
		if (!fluxAudio)
		{
            qWarning("Demande de mise en pause alors qu'aucun fichier n'est ouvert (joueurPauseMorceau - AudioPlayer.cpp)");
			return;
		}
		...
		// Dans tous les autres cas il s'agit d'une erreur
            ...
            qWarning("Demande de mis en pause du lecteur alors qu'il n'etait pas en lecture (joueurPauseMorceau - AudioPlayer.cpp)");
	}
    ...
    void AudioPlayer::joueurArretMorceau()
	{
		...
		if (!fluxAudio)
		{
            qWarning("Demande d'arret de lecture alors qu'aucun fichier n'est ouvert (joueurArretMorceau - AudioPlayer.cpp)");
			return;
		}
	...
	/* choisi par le MJ                                                 */
	/********************************************************************/
    void AudioPlayer::joueurChangerPosition(int position)
	{
		// Si aucun fichier audio n'est actuellement ouvert, on quitte la fonction
		if (!fluxAudio)
		{
            qWarning("Demande de changement de position de lecture alors qu'aucun fichier n'est ouvert (joueurChangerPosition - AudioPlayer.cpp)");
			return;
		}
	...
	/********************************************************************/
    void AudioPlayer::joueurChangerDossier()
	{
