...
void CampaignTest::copyCampaign_data()
{
    ...
    QTest::addRow("cmd2") << TupleMedia{{"girafe1", "girafe1", IOHelper::loadFile(":/img/girafe.jpg")}} << TupleDice{}
                          << TupleState{} << TupleNPC{};

    QTest::addRow("cmd3") << TupleMedia{{"girafe1", "girafe1", IOHelper::loadFile(":/img/girafe.jpg")}}
                          << TupleDice{{"pattern", "dicecommand", false}} << TupleState{} << TupleNPC{};

    QTest::addRow("cmd4") << TupleMedia{{"girafe1", "girafe1", IOHelper::loadFile(":/img/girafe.jpg")}}
                          << TupleDice{{"pattern", "dicecommand", false}}
                          << TupleState{{"state1", "state1", QColor(Qt::blue)}} << TupleNPC{};

    QTest::addRow("cmd5") << TupleMedia{{"girafe1", "girafe1", IOHelper::loadFile(":/img/girafe.jpg")}}
                          << TupleDice{{"pattern", "dicecommand", false}}
...
void CampaignTest::importFromAnotherCampaign()
{
    ...
    if(dir.isValid())
    {
        ...
        if(expectedEguality)
            ...
            QVERIFY(importedFile.size() != sourceListFile.size());
