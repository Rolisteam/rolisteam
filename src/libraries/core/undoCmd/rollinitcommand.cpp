...
{
    std::for_each(std::begin(m_data), std::end(m_data), [this](const QPointer<vmap::CharacterItemController>& item) {
        ...
        {
            diceparser->start();
            auto jsonstr= diceparser->resultAsJSon([](const QString& value, const QString&, bool) { return value; });
...
{
    std::for_each(std::begin(m_data), std::end(m_data), [](const QPointer<vmap::CharacterItemController>& item) {
        if(!item)
            return;
        auto character= item->character();
