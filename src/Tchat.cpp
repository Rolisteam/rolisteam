		...
		case '*':
            {
                ...
                if(exp.exactMatch(message))
                {
                    ...
                    {
                        messageCorps = tr("*12D ... ajoutez R pour rusher, G3 pour les Gremlins d'indice 3 et + pour relancer les 6 ... ajouter C pour ne pas afficher les dÃ©tails du lancer, et S pour n'afficher que les résultats.");
...
int Tchat::calculerJetDes(QString &message, QString &tirage, bool &ok)
{
	...
	while(message.size())
	{
		...
		// le nombre est suivi par un "G" : on recupere le nombre qui suit (nbr de des gardÃ©s)
