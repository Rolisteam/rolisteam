...
QString TextEdit::m_filter= QString("%1 %2 %3 %4")
                                .arg(tr("OpenOffice 2.4 file format OASIS "))
                                .arg(tr(" (*.odt *.ott);;"))
                                .arg(tr("XHTML file format"))
                                .arg(tr(" (*.htm *.html);;"));
...
TextEdit::TextEdit(QWidget* parent) : QMainWindow(parent)
{
    ...
    connect(textEdit, SIGNAL(currentCharFormatChanged(const QTextCharFormat&)), this,
...
{
    QString fn
        = QFileDialog::getOpenFileName(this, tr("Open File..."), QString(setter.value("LastDir").toString()), m_filter);
    if(!fn.isEmpty())
    {
        QFileInfo fi(fn);
