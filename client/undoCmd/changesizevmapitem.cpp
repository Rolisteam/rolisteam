...
ChangeSizeVmapItemCommand::ChangeSizeVmapItemCommand(VMap* vmap, RGraphicsView::Method method, QPointF mousePos,
                                                     QUndoCommand *parent)
    : QUndoCommand(parent),
      m_vmap(vmap)
{
    if(nullptr != m_vmap)
    {
        ...
        for(auto item : list)
        {
            ...
            if(vItem != nullptr)
            {
                if(!m_targetItem.contains(vItem) && vItem->isVisible()
                        && vItem->getType()!= VisualItem::GRID
                        && vItem->getType()!= VisualItem::SIGHT)
                {
