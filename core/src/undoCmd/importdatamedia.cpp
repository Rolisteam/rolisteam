...
namespace commands
{
ImportDataMedia::ImportDataMedia(campaign::Campaign* campaign, const QString& name, const QByteArray& data,
    ...
    , m_destPath(destPath)
{
    setText(tr("Import media: %1").arg(name));
    IOHelper::makeDir(m_destPath);
    m_destPath= QString("%1/%2").arg(m_destPath, name);
    IOHelper::writeFile(m_tmpPath, data);
}

void ImportDataMedia::redo()
{
